...
$table_name = 'student';
if (isset($_GET['search'])  and $_REQUEST['search'] != '') {
    $search = $_REQUEST['search'];
    $sql ="SELECT student.*, concat(student_admission, student_name,  student_mobile, student_class, student_section, student_roll) as  f1 FROM student having f1 like'%$search%'";
	$res =direct_sql($sql);
} 
else if (isset($_GET['link']) and $_GET['link'] != '') {
								...
								<tbody>
									...
									if ($res['count'] > 0) {
										foreach ($res['data'] as $row) {
											...
											$display_img = "<br><img src='required/upload/{$row['student_photo']}' height='30px' width='25px' >";
									?>
											...
											<tr>
											<?php } ?> 
												<td><?php echo $row['student_admission']. $display_img; ?></td>
												<td><a href="student_ledger.php?student_id=<?=$id?>&action=show" class='text-primary'><?php echo $row['student_name'] ?></a></td>
												...
												<td class='text-right'>
													...
													<a class='fa fa-print btn btn-dark btn-xs' href='student_profile?link=<?php echo $link; ?>' target='_op'></a>
