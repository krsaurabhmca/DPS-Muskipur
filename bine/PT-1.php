...
if(isset($_REQUEST['student_admission']) and $_REQUEST['student_admission'] !='')
	{
	    extract($_POST);
		$student_list = explode(",",$_REQUEST['student_admission']); 
	}
else{
    extract($_POST);
...
<tr>
	<td colspan='7' align='center' style='background:#525252;color:#fff;height:30px;'>
		<?php echo $exam_list[$exam_name] ." EXAMINATION " . $session_list[$db_name]; ?>
	</td>
</tr>
<tr>
...
<td colspan='7'>
	<table rules='none' width='100%' border='0' >
			...
			<td rowspan='3' align='center'>	<img src='upload/<?php echo get_data('student',$sid,'student_photo')['data']; ?>' width='60' height='60' class='photo'></td>
	...
	<td colspan='7' align='center' height='300px' valign='top'>
		<table rules='all' width='100%'  border='0' bordercolor='#c5c5c5'>
			...
			<tr>
				...
				$co_marks  = get_all('exam','*',['student_id'=>$sid,'exam_name'=>$exam_name])['data'][0];
			    //print_r($sub_list);
...
?>
<tr align='center'>
	<td colspan='2'><?php echo $exta_subject_name; ?></td>
...
<tr>
	...
	<th colspan='1'><?= $co_marks[remove_space($co)] ?></th>
...
<tr>
	<td colspan='5' valign='top' height='50px'>
			...
			<b>Remarks: <?= $co_marks['remarks'] ?></b>
	
	</td>
	<td colspan='150px' valign='top' align='center'><b>Attendance : <?= $co_marks['attendance'] ?></td>
</tr>
